...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
	...
	$currentcity = mysqli_real_escape_string($con, $_POST['currentcity']);
	...
	if (!mysqli_query($con, $query)) {
		print '<p class="error">Error: Failed to update user profile. ' . mysqli_error($con). '</p>';
	}
	
	/* should we add an interest? */
	if (!empty($_POST['add_interest']) and $_POST['add_interest'] != '(add interest)' and trim($_POST['add_interest']) != '') {
		
		$interest = mysqli_real_escape_string($con,$_POST['add_interest']);
		$query = "INSERT INTO userinterests (email, interest) " .
				 "VALUES('{$_SESSION['email']}', '$interest')";
		
		if (!mysqli_query($con,$query)) {
			print '<p class="error">Error: Failed to add interest. ' . mysqli_error($con). '</p>';
...
if (!empty($_GET['delete_interest'])) {
	...
	$interest = mysqli_real_escape_string($con,$_GET['delete_interest']);
	$query = "DELETE FROM userinterests " .
			 "WHERE email = '{$_SESSION['email']}' " .
			 "AND interest = '$interest'";
	
	if (!mysqli_query($con,$query)) {
		print '<p class="error">Error: Failed to delete interest. ' . mysqli_error($con). '</p>';
	}
}
